...
function renderResult() {
  ...
  db.collection("transaction").get().then((snapshots) => {
    ...
    snapshots.docs.forEach(doc => {
      ...
      let walletID = doc.data().walletID;
      ...
      var yearData =  date.getFullYear();
        if(wallet === walletID) {
          if(category === "food" && yearData === year && month === 1) {
            Jan += amount;         
          }
          if(category === "food" && yearData === year && month === 2) {
            Feb += amount;         
          }
          if(category === "food" && yearData === year && month === 3) {
            Mar += amount;         
            }
          if(category === "food" && yearData === year && month === 4) {
            Apr += amount;         
          }
          if(category === "food" && yearData === year && month === 5) {
            May += amount;         
          }
          if(category === "food" && yearData === year && month === 6) {
            Jun += amount;         
          }
          if(category === "food" && yearData === year && month === 7) {
            Jul += amount;         
          } 
          if(category === "food" && yearData === year && month === 8) {
            Aug += amount;         
          } 
          if(category === "food" && yearData === year && month === 9) {
            Sep += amount;         
          } 
          if(category === "food" && yearData === year && month === 10) {
            Oct += amount;         
          }
          if(category === "food" && yearData === year && month === 11) {
            Nov += amount;         
          }
          if(category === "food" && yearData === year && month === 12) {
            Dec += amount;         
          }   
   ...
   function renderChart() {
    ...
    var myLineChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: [`1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`, `1/${year}`],
        datasets: [{
          label: "Expense",
